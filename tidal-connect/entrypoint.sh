...
# Check if /0-entrypoint.sh script exists and execute it if present
if [ -f "/0-entrypoint.sh" ]; then
  info "Found /0-entrypoint.sh, executing it..."
  exec /0-entrypoint.sh

# If the script does not exist, continue with the rest of the entrypoint
else
  info "No /0-entrypoint.sh found, continuing with normal entrypoint execution."
